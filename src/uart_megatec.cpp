...
int8_t getMegatecUPSMetric(const uint8_t _rxPin, const uint8_t _txPin, char* _command, uint8_t _fieldNumber, char* _dst) {
  ...
  len = serialRecive(&swSerial, (uint8_t*) _dst, MEGATEC_MAX_ANSWER_LENGTH, MEGATEC_DEFAULT_READ_TIMEOUT, UART_STOP_ON_CHAR, '\r', !UART_SLOW_MODE);
  // __DMLD( DEBUG_PORT.print("Recieved "); DEBUG_PORT.print(len); DEBUG_PORT.println(" chars"); )
  // Answer must start with '(' or '#'
  if (expectedStartByte != _dst[0x00]) { rc = DEVICE_ERROR_WRONG_ANSWER; goto finish; }
  len--;
  ...
  srcPos = 0x01; // start from 1-th byte to skip '(' prefix
  fileldNumber = dstPos = 0x00;
