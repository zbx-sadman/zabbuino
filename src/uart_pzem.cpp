...
int8_t getPZEM004Metric(const uint8_t _rxPin, const uint8_t _txPin, const char* _ip, uint8_t _metric, int32_t* _value) {
  ...
  data[5] = 0x00; 
  // 7-th byte - CRC
  data[6] = crcPZEM004(data, PZEM_PACKET_SIZE - 1); 
  ...
  if (data[0] != (command - 0x10)) { rc = DEVICE_ERROR_WRONG_ANSWER; goto finish; }
  // Bad CRC
  if (data[6] != crcPZEM004( data, len - 1)) { rc = DEVICE_ERROR_CHECKSUM; goto finish; }
  ...
  switch (_metric) {
    ...
    case SENS_READ_VOLTAGE: {
      ...
      *_value = (((int32_t) data[1] << 8) + data[2]) * 10 + data[3]; 
    ...
    case SENS_READ_POWER: {
      ...
      *_value = ((uint16_t) data[1] << 8) + data[2];
    ...
    case SENS_READ_ENERGY: {
      ...
      *_value = ((int32_t) data[1] << 16) + ((uint16_t) data[2] << 8) + data[3];
      rc = RESULT_IS_UNSIGNED_VALUE;
      break;
    }
    ...
    case SENS_CHANGE_ADDRESS: {
      ...
      *_value = data[1] | data[2] | data[3] | data[4] | data[5];
