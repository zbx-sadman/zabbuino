...
*
*****************************************************************************************************************************/
int8_t initDS3231(const uint8_t _sdaPin, const uint8_t _sclPin, const uint8_t _i2cAddress) {
  int8_t rc = false;

  // Kickstart oscillator of it stopped early
  if (!isDS3231Running(_i2cAddress)) { setDS3231RunningState(_i2cAddress, true); }

  uint8_t creg;

  if (0x00 != readBytesFromi2C(_i2cAddress, DS3231_REG_CONTROL, &creg, 1)) { goto finish; }
  ...
  if (0x00 != writeByteToI2C(_i2cAddress, DS3231_REG_CONTROL, creg)) { goto finish; } 
...
*
*****************************************************************************************************************************/
static uint8_t isDS3231Running(const uint8_t _i2cAddress) {
  uint8_t creg;
...
*
*****************************************************************************************************************************/
static void setDS3231RunningState(const uint8_t _i2cAddress, uint8_t _isRunning) {
  uint8_t creg;
  readBytesFromi2C(_i2cAddress, DS3231_REG_CONTROL, &creg, 1);
  if (_isRunning) {
...
*
*****************************************************************************************************************************/
static uint8_t isDS3231DateTimeValid(const uint8_t _i2cAddress) {
...
*
*****************************************************************************************************************************/
int8_t saveDS3231Time(const uint8_t _sdaPin, const uint8_t _sclPin, uint8_t _i2cAddress, time_t _y2Ktimestamp) {
  int8_t rc = false;
  uint8_t status, value[DS3231_REG_TIMEDATE_SIZE];
  uint8_t year, centuryFlag;
  tm dateTime;

  if (false == isI2CDeviceReady(_i2cAddress)) { goto finish;  }
  ...
  rc = (0x00 == writeBytesToI2C(_i2cAddress, DS3231_REG_TIMEDATE, value, sizeof(value)));
...
*
*****************************************************************************************************************************/
int8_t readDS3231Time(const uint8_t _sdaPin, const uint8_t _sclPin, uint8_t _i2cAddress, time_t* _timestamp) {
  int8_t rc = false;
  ...
  if (!isDS3231DateTimeValid(_i2cAddress)) { goto finish; } // rc already init as RESULT_IS_FAIL

  rc = (0x00 == readBytesFromi2C(_i2cAddress, DS3231_REG_TIMEDATE, value, sizeof(value)));
